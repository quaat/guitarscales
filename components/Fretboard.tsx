...
import { ScaleCalculatedData, LabelMode, AccidentalMode } from '../types';
...
export const Fretboard: React.FC<FretboardProps> = ({ data, labelMode, accidentalMode }) => {
  ...
  const getNoteInfo = (stringIndex: number, fretIndex: number) => {
    ...
    return { isInScale, isRoot, degree, noteName, currentPitch };
  };
             ...
             {Array.from({ length: TOTAL_FRETS }).map((_, i) => (
               ...
               >
                 ...
                 {MARKERS.includes(i) && i !== 12 && (
                    <div className="absolute top-1/2 left-1/2 transform translate-x-[50%] -translate-y-1/2 w-3 h-3 rounded-full bg-slate-800 shadow-inner z-0"></div>
                 )}
                 {/* Double Dots for 12th fret */}
                 {i === 12 && (
                    <>
                      <div className="absolute top-[35%] left-1/2 transform translate-x-[50%] -translate-y-1/2 w-3 h-3 rounded-full bg-slate-800 shadow-inner z-0"></div>
                      <div className="absolute top-[65%] left-1/2 transform translate-x-[50%] -translate-y-1/2 w-3 h-3 rounded-full bg-slate-800 shadow-inner z-0"></div>
                    </>
                 )}
               </div>
             ))}
            ...
            return (
              <div key={stringIndex} className="relative flex items-center pl-[3rem]">
                ...
                {Array.from({ length: TOTAL_FRETS }).map((_, fretIndex) => {
                   const { isInScale, isRoot, degree, noteName } = getNoteInfo(stringIndex, fretIndex);
